<template>
  <div>
    ...
    <table v-else class="tabella-utenti">
      <thead><tr><th>Autore</th><th>Titolo</th><th>Corpo</th><th>Creato il</th></tr></thead>
      <tbody>
        <tr v-for="(p, i) in post" :key="p.id" class="riga-entrata" :style="{ animationDelay: `${i * 0.05}s` }">
          <td>Utente #{{ p.userId }}</td>
          <td class="td-nome">{{ p.titolo }}</td>
          <td class="td-corpo">{{ p.corpo }}</td>
          <td class="td-data td-data-multi">
            <span class="data-giorno">{{ fmt(p.creatoIl) }}</span>
            <span class="data-ora">{{ fmtOra(p.creatoIl) }}</span>
          </td>
        </tr>
      </tbody>
    </table>
...
onMounted(async () => {
  try { const r = await ottieniPost(); post.value = r.dati ?? r }
  catch (e) { errore.value = e.message }
  finally { caricamento.value = false }
})
const fmt = ts => new Date(ts).toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' })
